...
export class ChiBoService {
    ...
    public insert(obj:ChiBo): Promise<void> {
        ...
        return TheDb.insert(sql, values)
            .then((result) => {
                if (result.changes !== 1) {
                    throw new Error(`Expected 1 chibo to be inserted. Was ${result.changes}`);
                } else {
    ...
    public update(obj:ChiBo): Promise<void> {
        ...
        return TheDb.update(sql, values)
            .then((result) => {
                if (result.changes !== 1) {
                    throw new Error(`Expected 1 chibo to be updated. Was ${result.changes}`);
                }
            });
    }
    ...
    public delete(id:number): Promise<void> {
        ...
        return TheDb.delete(sql, values)
            .then((result) => {
                if (result.changes !== 1) {
                    throw new Error(`Expected 1 chibo to be deleted. Was ${result.changes}`);
                }
            });
    }
